...
[McpServerToolType]
public sealed class CorTools(CorService corService)
{
    ...
    [Description(
        ...
        "Use only when the user explicitly wants COR validation.")]
    public Task<string> ValidateCorGateway(
        [Description("COR gateway DTO as JSON.")]
        string dto,
        CancellationToken cancellationToken = default)
        => McpToolRunner.RunAsync(
            () => corService.ValidateCorGatewayAsync(dto, cancellationToken),
            cancellationToken);
}
